...
class linear_cca():
    ...
    def fit(self, H1, H2, outdim_size):
        ...
        m = H1.shape[0]# 视图1的样本数量
        o1 = H1.shape[1]# 视图1的原特征数
        o2 = H2.shape[1]# 视图2的原特征数

        self.m[0] = numpy.mean(H1, axis=0)# 压缩行，对各列求均值，返回 1* n 矩阵
        self.m[1] = numpy.mean(H2, axis=0)
        H1bar = H1 - numpy.tile(self.m[0], (m, 1))# 在列方向上重复
