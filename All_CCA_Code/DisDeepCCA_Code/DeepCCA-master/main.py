...
class Solver():
    def __init__(self, model, Discriminal_cca, outdim_size, epoch_num, batch_size, learning_rate, reg_par, device=torch.device('cpu')):
        ...
        self.Discriminal_cca = Discriminal_cca
    ...
    def fit(self, x1, x2, target_train, target_v, target_t, vx1=None, vx2=None, tx1=None, tx2=None, checkpoint='checkpoint.model'):
        """
        ...
        for epoch in range(self.epoch_num):
            ...
            for batch_idx in batch_idxs:
                ...
                batch_target_train = [target_train[i] for i in batch_idx]
                o1, o2 = self.model(batch_x1, batch_x2)
                loss = self.loss(o1, o2, batch_target_train)
            ...
            if vx1 is not None and vx2 is not None:
                with torch.no_grad():
                    self.model.eval()
                    val_loss, _ = self.test(vx1, vx2, target_v)
        ...
        # train_Discriminal_cca
        if self.Discriminal_cca is not None:
            _, outputs = self._get_outputs(vx1, vx2, target_v)
            #self.train_Discriminal_cca(outputs[0], outputs[1], target_v)

        checkpoint_ = torch.load(checkpoint)
        self.model.load_state_dict(checkpoint_)
        if vx1 is not None and vx2 is not None:
            loss, _ = self.test(vx1, vx2, target_v)
            self.logger.info("loss on validation data: {:.4f}".format(loss))

        if tx1 is not None and tx2 is not None:
            loss, _ = self.test(tx1, tx2, target_t)
            self.logger.info('loss on test data: {:.4f}'.format(loss))

    def test(self, x1, x2, target):
        with torch.no_grad():
            losses, outputs = self._get_outputs(x1, x2, target)
            return np.mean(losses), outputs

    def train_Discriminal_cca(self, x1, x2, target):
        self.Discriminal_cca.fit(x1, x2, target, self.outdim_size)

    def _get_outputs(self, x1, x2, target):
        with torch.no_grad():
            self.model.eval()
            data_size = x1.size(0)
            batch_idxs = list(BatchSampler(RandomSampler(
            ...
            for batch_idx in batch_idxs:
                ...
                outputs1.append(o1)
                outputs2.append(o2)
                loss = self.loss(o1, o2, batch_target)
...
if __name__ == '__main__':
    ...
    # if a Discriminal CCA should get applied on the learned features extracted from the networks
    # it does not affect the performance on noisy MNIST significantly
    apply_Discriminal_cca = False
    # end of parameters section
    ############

    # Building, training, and producing the new features by DisDCCA
    model = DeepCCA(layer_sizes1, layer_sizes2, input_shape1,
                    input_shape2, outdim_size, use_all_singular_values, device=device).double()
    D_cca = None
    solver = Solver(model, D_cca, outdim_size, epoch_num, batch_size,
                    learning_rate, reg_par, device=device)

    solver.fit(train1, train2, target_train, target_v, target_t, val1, val2, test1, test2)

    result_train = solver.predict(train1, train2)
    result_val = solver.predict(val1, val2)
    result_test = solver.predict(test1, test2)

    outputs_disdcca0 = np.concatenate((result_train[0], result_val[0], result_test[0]))
    outputs_disdcca1 = np.concatenate((result_train[1], result_val[1], result_test[1]))
    ...
    print('training SVM...')
    for batch_idx in batch_idxs:
        batch_x1 = outputs_disdcca0[batch_idx, :]
        batch_x2 = outputs_disdcca1[batch_idx, :]
        batch_target = [target[i] for i in batch_idx]
        ...
        vacc_disdcca1.append(valid_acc1)
        tacc_disdcca1.append(test_acc1)
        vacc_disdcca2.append(valid_acc2)
        tacc_disdcca2.append(test_acc2)
    ...
    c=np.concatenate((outputs_disdcca0 , outputs_disdcca1))
    ...
    for batch_idx in batch_idxs:
        ...
        vacc_disdcca.append(valid_acc)
        tacc_disdcca.append(test_acc)

    vacc_disdcca_mean = np.mean(vacc_disdcca)
    vacc_disdcca_var = np.var(vacc_disdcca)
    tacc_disdcca_mean = np.mean(tacc_disdcca)
    tacc_disdcca_var = np.var(tacc_disdcca)
    vacc_disdcca1_mean = np.mean(vacc_disdcca1)
    vacc_disdcca1_var = np.var(vacc_disdcca1)
    tacc_disdcca1_mean = np.mean(tacc_disdcca1)
    tacc_disdcca1_var = np.var(tacc_disdcca1)
    vacc_disdcca2_mean = np.mean(vacc_disdcca2)
    vacc_disdcca2_var = np.var(vacc_disdcca2)
    tacc_disdcca2_mean = np.mean(tacc_disdcca2)
    tacc_disdcca2_var = np.var(tacc_disdcca2)

    #print("Accuracy on view 1 (validation data) is:", vacc1_mean * 100.0, "+-", vacc1_var * 100.0)
    print("DisDCCA Accuracy on view 1 (test data) is:", tacc_disdcca1_mean*100.0, "+-", tacc_disdcca1_var * 100.0)
    #print("Accuracy on view 2 (validation data) is:", vacc2_mean * 100.0, "+-", vacc2_var * 100.0)
    print("DisDCCA Accuracy on view 2 (test data) is:", tacc_disdcca2_mean*100.0, "+-", tacc_disdcca2_var * 100.0)
    #print("Accuracy (validation data) is:", vacc_mean * 100.0, "+-", vacc_var * 100.0)
    print("DisDCCA Accuracy (test data) is:", tacc_disdcca_mean*100.0, "+-", tacc_disdcca_var * 100.0)
